...
sem_datos = int(max_fecha.isocalendar()[1])

archivos_meta = sorted(glob.glob(os.path.join(metas_dir, "*.xls*")))
elegir_meta = None
meta_pattern = re.compile(r"meta[_\-]?Sem[_\-]?(\d{1,2})", re.IGNORECASE)
for mf in archivos_meta:
    m = meta_pattern.search(os.path.basename(mf))
    if m and int(m.group(1)) == sem_datos:
        elegir_meta = mf
        break
if elegir_meta is None and archivos_meta:
    elegir_meta = max(archivos_meta, key=os.path.getmtime)
...
if elegir_meta:
    try:
        df_meta_orig = pd.read_excel(elegir_meta)
        ...
        st.warning(f"Error al cargar metas: {e}")
...
def crear_grafica_sem(df_input, tipo_filter=None, title="MONTO POR DÍA Y SEMANA"):
    ...
    fig = go.Figure()
    fig.add_trace(
        go.Bar(x=labels, y=monto_prev, name=f"Semana {dias_sem[0].isocalendar()[1] - 1}", marker_color="#AED6F1"))
    fig.add_trace(go.Bar(x=labels, y=monto_curr, name=f"Semana {dias_sem[0].isocalendar()[1]}", marker_color="#2980B9"))
    fig.add_trace(go.Scatter(x=labels, y=meta_curr, name="META", mode="lines+markers",
                             line=dict(color="#27AE60", width=3, shape="spline"), marker=dict(color="#27AE60", size=6)))
    fig.update_layout(title=title, barmode="group", yaxis=dict(title="Monto", tickformat="$,.0f"),
                      template="plotly_white", height=400, legend=dict(yanchor="top", y=0.99, xanchor="left", x=0.01),
...
def crear_grafica_sem_cases(df_input, tipo_filter=None, title="CASOS POR DÍA Y SEMANA"):
    ...
    fig = go.Figure()
    fig.add_trace(
        go.Bar(x=labels, y=casos_prev, name=f"Semana {dias_sem[0].isocalendar()[1] - 1}", marker_color="#AED6F1"))
    fig.add_trace(go.Bar(x=labels, y=casos_curr, name=f"Semana {dias_sem[0].isocalendar()[1]}", marker_color="#2980B9"))
    fig.add_trace(go.Scatter(x=labels, y=meta_curr, name="META", mode="lines+markers",
                             line=dict(color="#27AE60", width=3, shape="spline"), marker=dict(color="#27AE60", size=6)))
    fig.update_layout(
        title=title, barmode="group", yaxis=dict(title="Casos", tickformat=","), template="plotly_white", height=400,
...
st.markdown(
    ...
    .block-container .css-1v3fvcr, /* selector común en streamlit */
  ...
  color: #ffffff !important;
    ...
    :root{
      --grad-start: #000046;
      --grad-end:   #1CB5E0;
      --card-bg: rgba(0,0,0,0.28);     /* fondo de tarjetas (semi-transparente) */
      --input-bg: rgba(255,255,255,0.06); /* fondo inputs/selects pero con texto blanco */
    ...
    /* Forzamos color blanco en textos dentro de Streamlit */
...
logo_ruta = r"\\fc-fs01\Planeacion\Michel\logo_fincomun.png"
st.markdown(
    f"""
    <div class="header-container">
        <img src="data:image/png;base64,{base64.b64encode(open(logo_ruta, 'rb').read()).decode()}" width="180" style="margin-right: 80px;">
...
with col_zona_nuevos:
    ...
    if not df_zona_nuevos.empty:
        ...
        df_zona_nuevos["ICONO"] = df_zona_nuevos["LOGRO_PCT"].apply(
            lambda x: "✅" if pd.notna(x) and x >= 100 else "🟡" if pd.notna(x) and x >= 80 else "🔴" if pd.notna(
                x) else "")
        ...
        st.dataframe(df_zona_nuevos[["ZONA", "MONTO", "CASOS", "META", "ICONO", "LOGRO_PCT"]], hide_index=True,
...
with col_sucursal_nuevos:
    ...
    if not df_sucursal_nuevos.empty:
        ...
        df_sucursal_nuevos["LOGRO_PCT"] = (df_sucursal_nuevos["MONTO"] / df_sucursal_nuevos["META"].replace(
            {0: pd.NA})) * 100
        ...
        st.dataframe(df_sucursal_nuevos[["SUCURSAL", "MONTO", "CASOS", "META", "ICONO", "LOGRO_PCT"]], hide_index=True,
                     use_container_width=True)
    else:
        st.write("No hay datos para Nuevos.")
...
with col_zona_recomp:
    ...
    if not df_zona_recomp.empty:
        ...
        st.dataframe(df_zona_recomp[["ZONA", "MONTO", "CASOS", "META", "ICONO", "LOGRO_PCT"]], hide_index=True,
                     use_container_width=True)
    else:
...
with col_sucursal_recomp:
    ...
    if not df_sucursal_recomp.empty:
        ...
        df_sucursal_recomp["LOGRO_PCT"] = (df_sucursal_recomp["MONTO"] / df_sucursal_recomp["META"].replace(
            {0: pd.NA})) * 100
        ...
        st.dataframe(df_sucursal_recomp[["SUCURSAL", "MONTO", "CASOS", "META", "ICONO", "LOGRO_PCT"]], hide_index=True,
                     use_container_width=True)
    else:
        st.write("No hay datos para Recompra.")
...
with col_zona_total:
    ...
    if not df_zona_total.empty:
        ...
        st.dataframe(df_zona_total[["ZONA", "MONTO", "CASOS", "META", "ICONO", "LOGRO_PCT"]], hide_index=True,
                     use_container_width=True)
    else:
...
with col_sucursal_total:
    ...
    if not df_sucursal_total.empty:
        ...
        df_sucursal_total["LOGRO_PCT"] = (df_sucursal_total["MONTO"] / df_sucursal_total["META"].replace(
            {0: pd.NA})) * 100
        ...
        st.dataframe(df_sucursal_total[["SUCURSAL", "MONTO", "CASOS", "META", "ICONO", "LOGRO_PCT"]], hide_index=True,
                     use_container_width=True)
    else:
        st.write("No hay datos.")
